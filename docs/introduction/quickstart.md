...
Once tasks finish, the experiment directory will contain the generated files. Note that some tasks are required to succeed to triger the execution of other tasks while some tasks can fail without impacting cycling, if the cycl graph doesn't require completion of the task to move forward.
