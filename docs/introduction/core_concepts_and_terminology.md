...
The Swell system is developed by NASA's Global Modeling and Assimilation Office. It is used to generate experiments using the [JEDI](https://www.jcsda.org/jcsda-project-jedi) data assimilation system and NASA's Goddard Earth Observing System (GEOS) model.
...
- **Model-specific (non-generic) tasks** — tasks whose name and implementation are tied to one particular model component, because that model needs a distinct file format, restart handling, or run-directory layout. Currently in SWELL, this is done in two ways, first of which is by creating tasks with model names under the `/tasks` folder. For instance, `PrepForecastCf`, `SaveForecastCf`, `GetRestartCf`, and `SaveRestartCf` are specific to `geos_cf`, while `GetCoupledGeosRestart`, `LinkCoupledGeosOutput`, and `PrepCoupledGeosRunDir` are specific to the `coupled/marine` type applications. These tasks are only ever used by the suites that need that particular model. Second, there are tasks under model folders, such as `tasks/geos_marine/get_background_geos_marine.py`. When there is a generic and model-specific version of a task, SWELL will look for and use the model-specific task. In this example. `swell task GetBackground -m geos_marine` will execute the `task/geos_marine/get_background_geos_marine.py` script.
