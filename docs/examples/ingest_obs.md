...
### Step 1: Create a YAML file describing your observation

Create a file in `src/swell/configuration/jedi/interfaces/geos_marine/ingest_observations/` named after your observation (e.g., `my_obs.yaml`):
...
The path uses placeholders that get replaced with actual dates:
- `YYYY` → 2021
- `MM` → 07
- `DD` → 02
- `HH` → 06
...
### Step 3: Add your observation to the suite config

Edit `src/swell/suites/ingest_obs/suite_config.py`. If you're ingesting marine observations, update the `ingest_obs_marine` section. For other domains (atmosphere, etc.), you can create a similar section like `ingest_obs_atmosphere` following the same pattern.

Add your observation name to the list:

```python
qd.obs_to_ingest(['adt_cryosat2n', 'my_obs'])  # Add yours here
```

Also update the date range to match your files:
...
```bash
# Create the experiment using the suite name you configured
# (ingest_obs_marine for marine, or your custom name)
...
Each observation type has its own YAML in `configuration/jedi/interfaces/geos_marine/ingest_observations/`.
...
2. For each observation, loads its YAML config and looks up the provider
3. Checks R2D2 to see if this observation already exists (skips if it does)
4. Builds the file path by replacing date placeholders with the actual cycle time
5. Calls `r2d2.store()` to ingest the file with the right metadata
...
swell/src/swell/
├── suites/ingest_obs/
│   ├── flow.cylc              # Cylc workflow definition
│   ├── suite_config.py        # Default configuration
│   └── README.md              # This file
├── tasks/
│   └── ingest_obs.py          # IngestObs task implementation
└── configuration/jedi/interfaces/geos_marine/
    └── ingest_observations/   # Observation YAML configs
        ├── adt_cryosat2n.yaml
        ├── adt_sentinel6a.yaml
        └── ...
```
...
**"File not found"** - Check that your `cp_source` path pattern is correct and the files actually exist for your date range. Make sure the date placeholders match your file naming convention.
