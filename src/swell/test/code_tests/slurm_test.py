    ...
    def test_slurm_config(self, platform_mocked: Mock, mock_global_defaults: Mock) -> None:
        ...
        for sd in [sd_discover_sles15]:
            for mc in ["all", "geos_atmosphere", "geos_marine"]:
                run_jedi_var_obj = run_jedi_var_class(mc, 'nccs_discover_sles15')
                eva_obs_obj = eva_obs_class(mc, 'nccs_discover_sles15')
                build_jedi_obj = build_jedi_class(mc, 'nccs_discover_sles15')
