...
class GetBackground(taskBase):
  ...
  def execute(self):
    ...
    forecast_offset = cfg.get('analysis_forecast_window_offset') # Position relative to center of
                                                                 # the window where forecast starts
    ...
    for fc in r2d2_dict['fetch']['fc']:
      ...
      for file_type in fc['file_type']:
        ...
        for bkg_step in bkg_steps:
          ...
          target_file = os.path.join(cfg.get('experiment_dir'), current_cycle, target_file)
