...
class BuildJediByLinking(taskBase):
    ...
    def execute(self) -> None:
        ...
        if self.config.resolve(jedi_build_method) not in ('use_existing', 'use_pinned_existing'):
            self.logger.abort(f'Found \'{self.config.resolve(jedi_build_method)}\' for ' +
        ...
        if self.config.resolve(jedi_build_method) == 'use_existing':
            existing_jedi_build_directory = self.config.resolve(existing_jedi_build_directory)
        else:
            existing_jedi_build_directory = self.config.resolve(existing_jedi_build_directory_pinned)
