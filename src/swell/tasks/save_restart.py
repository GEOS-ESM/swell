...
class SaveRestart(taskBase):
    ...
    def execute(self):
        ...
        if model == 'geos_cf':
            ...
            local_background_time = self.da_window_params.local_background_time(
            ...
            # Oceanstats needs special handling from the forecast folder.
            # It is produced at the end of the forecast and could be saved as a good metric.
            # We are replicating the same structure as the R2D2 files.
            ...
            dst_stats = os.path.join(
                mainf,
                forecast_start_time.strftime('%Y-%m-%d'),
                (
                    f'mom6_cice6_UFS.{self.experiment_id()}.fc.global.MOM.oceanstats.'
                    f'{dst_date}.{forecast_duration}.nc'
                )
            )
