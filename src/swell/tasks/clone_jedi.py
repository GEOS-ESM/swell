...
class CloneJedi(taskBase):
    ...
    def execute(self) -> None:
        ...
        # ----------------------------------------------------------------
        if self.config.resolve(resolve, default=jedi_build_method) == 'use_existing':
            # Link the source code directory
            link_path(self.config.resolve(resolve, default=existing_jedi_source_directory), jedi_bundle_source_path)

        elif self.config.resolve(resolve, default=jedi_build_method) == 'use_pinned_existing':
            # Check hashes before proceeding
            check_hashes(self.config.resolve(resolve, default=existing_jedi_source_directory_pinned), self.logger)
            # Link the pinned source code directory
            link_path(self.config.resolve(resolve, default=existing_jedi_source_directory_pinned), jedi_bundle_source_path)

        elif self.config.resolve(resolve, default=jedi_build_method) in ('create', 'pinned_create'):
            ...
            use_pinned = False
            if self.config.resolve(resolve, default=jedi_build_method) == 'pinned_create':
                use_pinned = True

            # Generate the build dictionary
            jedi_bundle_dict = set_jedi_bundle_config(self.config.resolve(resolve, default=bundles, bundles),
