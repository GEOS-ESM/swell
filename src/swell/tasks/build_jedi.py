    ...
    if 'executable' in cfg.keys() and 'existing build directory' in cfg['build jedi'].keys():
      ...
      if os.path.exists(os.path.join(ex_build_dir, 'bin', cfg['executable'])):
        self.logger.info("Suitable JEDI build found, linking build directory. Warning: " +
                         "problems will follow if the loaded modules are not consistent with " +
                         "those used to build this version of JEDI.")
      ...
      # Prep ecbuild string
      ecb_tmp = 'ecbuild_bundle( PROJECT {} GIT "https://github.com/{}/{}.git" BRANCH {} UPDATE )\n'
      ...
      subprocess.run(['ecbuild -DMPIEXEC=$MPIEXEC {}'.format(bundle_dir)], shell=True) # Format string used to be '../' so we need to point it directly to bundle
      os.chdir('fv3-jedi/')
