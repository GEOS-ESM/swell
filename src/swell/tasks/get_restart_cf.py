...
import os
import r2d2
import r2d2.error as r2d2_error
...
class GetRestartCf(taskBase):
    ...
    def execute(self) -> None:
        ...
        rst_file_types_optional = self.config.rst_file_types_optional(default=[])
        ...
        for file_type in rst_file_types:
            ...
            except r2d2_error.R2D2Error as e:
                if file_type in rst_file_types_optional:
                    self.logger.info(
                        f'Optional restart file {file_type} not found in R2D2, skipping. '
                        f'GEOS will bootstrap this field. ({e})')
                    continue
                raise
