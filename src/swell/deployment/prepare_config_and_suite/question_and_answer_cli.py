...
class GetAnswerCli:
    ...
    def make_generic_widget(self, prompt: str, default, options: list, widget_type: WidgetType):
        ...
        answer = questionary.text(f"{prompt} [{default}]", validate=lambda text: True
                                  if widget_type.validate_value(text)
                                  else f'Please enter a value of type: {widget_type.value}',
                                  default=default).ask()
    ...
    def make_check_widget(self, prompt: str, default, options: list, widget_type: WidgetType):
        ...
        answer = questionary.checkbox(prompt, choices=choices, default=default,
                                      validate=lambda text: True
                                      if (widget_type.validate_value(text) and text != [])
                                      else 'Select at least one option').ask()
